...
import {
  checkStockForProcess,
  deleteProcess,
  getProcessInfo,
  getProcesses,
  registerIncomeForProcess,
} from "@api/Management";
...
export default function Manufactoring() {
  ...
  const request = useCallback(() => {
    getProcesses().then((res) => setProcesses(res));
  }, []);

  const processInfoRequest = (id: ManufacturingProcess["id"]) => {
    getProcessInfo(id).then((res) => setProcessInfo(res));
  };
  ...
  const handleShowCloseModal = useCallback(
    (show: boolean, id?: ManufacturingProcess["id"]) => {
      if (show && id) {
        setModalIsOpen(true);
  ...
  const handleAddProduct = useCallback(
    (name: string, techCard: TechCard, quantity: number, boxing: Boxing[]) => {
      console.log(
        "name",
        name,
        "techCard",
  ...
  const rowActions: WithTableActionsProps<ManufacturingProcess>["getRowActions"] =
    useCallback(
      (item) => {
        const actionDelete = {
          text: "Удалить",
          ...
          handler: (item: ManufacturingProcess) => {
            deleteProcess(item.id).finally(() => request());
          },
        };
        ...
        if (
          item.stepName === "Сформировать задачу на производство" ||
          item.stepName === "Учесть готовую продукцию на складе"
        ) {
  ...
  return (
    <div className={styles.pageWrapper}>
      <AddProcessModal
        ...
        onAddItem={(item: TechCard, quantity: number) => {
          console.log(item, quantity);
        }}
        onAddProduct={handleAddProduct}
      />
      <Modal
        open={modalIsOpen && productIncome?.id !== undefined}
        ...
        <div className={styles.modal}>
          ...
          <TextInput
            ...
            onChange={(e) => {
              ...
              if (productIncome?.id)
                setProductIncome({ ...productIncome, stock });
            }}
          />
      ...
      <div className={styles.header}>
        <div className={styles.headerTitle}>Производство</div>
        <Button size="l" view="action" onClick={() => setAddModalIsOpen(true)}>
          Добавить
        </Button>
      </div>
